...
with tab20:
    ...
    fig_fixe.update_layout(yaxis_ticksuffix='€', yaxis_tickformat=',', separators='.,', showlegend=True,
                            legend = dict(title=None, orientation='h',yanchor='bottom', y=1.15, xanchor='center', x=0))
    st.plotly_chart(fig_fixe, use_container_width=False)
...
# df_lags = {}
# for servei in serveis:
#     df_lags[servei] = add_lags(dict_serveis[servei])
#     st.write(servei)
#     st.write(df_lags[servei])



# prueba = add_lags(lloguer)
# prueba2 = add_lags(lloguer_year)
# st.write(prueba)
# st.write(prueba2)

# models_serveis, fi, scores_mean, scores_serveis, preds_serveis = generate_serveis_models(dict_serveis, plot=True, n_splits=5, test_size=2)

# st.write(models_serveis['Electricitat'].get_params())
...
with tab40:
    st.markdown('### **Models**')
    models_cat, fi, scores_mean, scores_cat, preds_cat = generate_cat_models(mf_cat, plot=True, n_splits=3, test_size=12)
